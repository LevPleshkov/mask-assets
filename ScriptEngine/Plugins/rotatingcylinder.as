...
class rotatingcylinder : BasePlugin
{
    String pluginName = "rotatingcylinder";

    String __effect_tag = "";
    bool __follow_head_tilt = false;
    float __speed = 1.0;
    ...
    void LoadSettings(const JSONValue& plugin_config) override
    {
        if (plugin_config.Get("name").GetString().ToLower() == pluginName)
        {
            ...
            if (plugin_config.Contains("follow_head_tilt"))
                __follow_head_tilt = plugin_config.Get("follow_head_tilt").GetBool();
